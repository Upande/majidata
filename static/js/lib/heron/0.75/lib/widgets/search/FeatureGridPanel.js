    ...
    setupStore: function (features) {
        ...
        if (this.autoConfig && features) {
            ...
            for (var i = 0; i < features.length; i++) {
				...
				for(var i=0;i<colabels.length;i++){					
					if( fieldName.toLowerCase() == colabels[i]['oldname']){
						fieldName= colabels[i]['newname'];					
					}
				}
                    // Capitalize header names
                    var column = {
                        //header: this.columnCapitalize ? fieldName_.substr(0, 1).toUpperCase() + fieldName_.substr(1).toLowerCase() : fieldName_,
                        header: fieldName,
						width: 100,
                        dataIndex: fieldName_,
                        sortable: true
                    };
